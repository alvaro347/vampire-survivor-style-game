...
func tween_collect(percent: float, start_position: Vector2) -> void:
	...
	rotation = lerp_angle(rotation, target_rotation, 1 -exp(-2 * get_physics_process_delta_time()))
...
func on_area_entered(ohter_area: Area2D) -> void:
	# NOTE: call on the next idle frame
	Callable(disable_collision).call_deferred()
	var tween: Tween = create_tween()
	tween.set_parallel() # Runs everything in parrallel
	# NOTE: Store a snapshot of the position of the vial
